...
Machine::Machine(int code, char* name, char* recipe,double price)
{
	...
	strcpy_s(this->name, strlen(recipe) + 1, recipe);
	this->price = price;
}
...
{
	return this->code == obj.code && strcmp(this->name, obj.name) == 0 && this->price == obj.price and this->price == obj.price;
}
...
{
	os << obj.code << " " << obj.name << " " << obj.price << " " << obj.recipe;
	return os;
}
